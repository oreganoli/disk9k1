...
fn find_user(conn: &PgConnection, username: &str, password: &str,) -> Result<Option<User>, AuthenticationError> {
...
fn register_user(conn: &PgConnection, username: &str, password: &str,) -> Result<User, AuthenticationError> {
...
fn get_username() -> Result<String, AuthenticationError> {
    if_not_present(dotenv::var("FILE_USERNAME"), NoUsernameSet)
}

fn get_password() -> Result<String, AuthenticationError> {
    if_not_present(dotenv::var("FILE_PASSWORD"), NoPasswordSet)
}

fn if_not_present<T>(res: Result<T, dotenv::Error>, on_not_present: AuthenticationError,) -> Result<T, AuthenticationError> {
...
mod tests {
    ...
    fn current_user_from_env_fails_when_no_password_set() {
        let _guard = this_test_modifies_env();
        env::remove_var("FILE_PASSWORD");
        env::set_var("FILE_USERNAME", "adasumizox");
    ...
    fn current_user_returns_none_when_no_user_exists_with_username() {
        ...
        assert_matches!(find_user(&conn, "adasumizox", "hunter"), Ok(None));
    }
    ...
    fn current_user_returns_the_user_if_it_has_the_same_password() {
        ...
        let expected_user = register_user(&conn, "adasumizox", "hunter").unwrap();
        let user = find_user(&conn, "adasumizox", "hunter").unwrap();

        asert_eq!(Some(exptected_user), user);
    }
    ...
    fn current_user_fails_if_password_does_not_match() {
        ...
        register_user(&conn, "adasumizox", "letmein").unwrap();
        let result = find_user(&conn, "adasumizox", "hunter");
